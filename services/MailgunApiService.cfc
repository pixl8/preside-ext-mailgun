...
// PUBLIC API METHODS
	/**
	 * Attempts to send a message through the MailGun API - returns message if successul and throws an error otherwise
	 *
	 */
	...
	private string function _getDefaultDomain(){
		return _getSettings().mailgun_default_domain;
	}


	private boolean function _getForceTestMode(){
		var testMode = _getSettings().mailgun_test_mode;
